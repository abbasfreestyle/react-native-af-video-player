...
class Video extends Component {
		...
		this.setState({ renderError: true }, () => {
			...
			switch (true) {
				...
				case typeof error === 'object':
					...
					break;
				default:
					type = Alert.alert('Oops!', 'There was an error playing this video, please try again later.', [
						{ text: 'Close' },
					]);
					break;
			}
	...
	animToFullscreen(height) {
		this.animFullscreen = new Animated.Value(0);
		Animated.parallel([
	...
	renderError() {
		...
		return (
			<Animated.View style={[styles.background, fullScreen ? styles.fullScreen : inline]}>
				...
				<Icons name='replay' size={60} color={this.props.theme} onPress={() => this.setState({ renderError: false })} />
			</Animated.View>
		);
	}

	renderPlayer() {
		const { fullScreen, paused, muted, loading, progress, duration, inlineHeight, currentTime } = this.state;
